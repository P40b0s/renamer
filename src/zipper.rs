...
/// 
fn zip_recursive(progress_bar: &indicatif::ProgressBar, zip_root_dir: &str,  zip: &mut ZipWriter<File>, zip_file_options: FileOptions<'_, ()>, src: impl AsRef<Path>, dst: impl AsRef<Path>) -> zip::result::ZipResult<()>
{
    let src = src.as_ref();
    for entry in std::fs::read_dir(src)? 
    {
        ...
        if ty.is_dir() 
        {
            ...
            zip.add_directory(&relative_dir_path, zip_file_options)?;
            zip_recursive(progress_bar, zip_root_dir, zip, zip_file_options, entry.path(), dst.as_ref().join(entry.file_name()))?;
        ...
        else 
        {
            ...
            zip.start_file(relative_path.1, zip_file_options)?;
...
pub fn zip_packet(progress_bar: &indicatif::ProgressBar, packet_name: &str, src: impl AsRef<Path>, dst: impl AsRef<Path>) -> zip::result::ZipResult<()>
{
    ...
    if let Some(zip_root_dir) = src.iter().last().and_then(|l| l.to_str())
    {
        ...
        //logger::info!("{}", &target_file.display());
        ...
        zip_recursive(progress_bar, zip_root_dir, &mut zip, options, src, trg)?;
        zip.finish()?;
    }
    else 
    {
